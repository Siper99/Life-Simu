...
vi.mock("./client", () => ({
  ...
  extractJson: vi.fn((raw: string) => JSON.parse(raw)),
}));
import { chat } from "./client";
import { narrateSceneBeat, narrateTurn, parseIntents } from "./orchestrator";
...
describe("双后端内容路由：常规走主叙事后端，露骨只走 nsfw 后端", () => {
  ...
  function mkOutcome(nsfw: boolean, summary = "与恋人共度良宵"): TurnOutcome {
    return {
      actions: [{
        intent: {
          id: "a", summary, category: "romance", hours: 10,
  ...
  it("双信号防误染：LLM 乱标 nsfw 但文本毫无露骨信号 → 仍走主叙事后端", async () => {
    mockedChat.mockClear();
    const { state } = newGameState(1);
    const res = await narrateTurn(
      explicitSettings([official, local]),
      state,
      "认真约会，一起看电影",
      mkOutcome(true, "和恋人看一场电影"), // 被解析器误标 nsfw 的普通约会
    );
    expect(mockedChat.mock.calls[0][0]).toBe(official);
    expect(res.nsfw).toBe(false);
  });

  it("意图解析按输入信号路由：普通输入走主叙事，露骨输入才走 nsfw 后端", async () => {
    const intentJson = '[{"summary":"认真学习","category":"study","hours":10,"risk":"low","attr":"intelligence"}]';
    mockedChat.mockClear();
    mockedChat.mockResolvedValueOnce(intentJson);
    const { state } = newGameState(2);
    const settings = explicitSettings([official, local]);
    await parseIntents(settings, state, "认真学习，周末去爬山");
    expect(mockedChat.mock.calls[0][0]).toBe(official); // 正常输入不再无脑发 Grok

    mockedChat.mockClear();
    mockedChat.mockResolvedValueOnce(intentJson);
    await parseIntents(settings, state, "和小柔翻云覆雨一整晚");
    expect(mockedChat.mock.calls[0][0]).toBe(local); // 带露骨信号才走 nsfw 后端
  });
});
...
describe("意图解析的金钱现实感", () => {
  ...
  it("moneyCost 按真实标价透传（不替玩家降价），付不付得起由引擎把关", async () => {
    mockedChat.mockClear();
    mockedChat.mockResolvedValueOnce(
      '[{"summary":"去买部新手机","category":"leisure","hours":5,"risk":"low","attr":"luck","moneyCost":3500}]',
    );
    const { state } = newGameState(6);
    state.character.money = 400; // 明显买不起，也不该在解析层被压价
    const { intents, usedLlm } = await parseIntents({ ...DEFAULT_SETTINGS, profiles: [official] }, state, "去买部新手机");
    expect(usedLlm).toBe(true);
    expect(intents[0].moneyCost).toBe(3500);
  });

  it("荒唐的天价被裁剪到上限，非法值被忽略", async () => {
    mockedChat.mockClear();
    mockedChat.mockResolvedValueOnce(
      '[{"summary":"买下一座城市","category":"finance","hours":5,"risk":"high","attr":"luck","moneyCost":999999999999},{"summary":"散步","category":"leisure","hours":2,"risk":"low","attr":"mood","moneyCost":-50}]',
    );
    const { state } = newGameState(6);
    const { intents } = await parseIntents({ ...DEFAULT_SETTINGS, profiles: [official] }, state, "x");
    expect(intents[0].moneyCost).toBe(10_000_000);
    expect(intents[1].moneyCost).toBeUndefined();
  });
});
...
describe("露骨信号词", () => {
  it("认得明确的性内容，放过正常的恋爱与生活", async () => {
    const { looksExplicit } = await import("./prompts");
    expect(looksExplicit("和小柔翻云覆雨")).toBe(true);
    expect(looksExplicit("两人在酒店开房过夜")).toBe(true);
    expect(looksExplicit("认真约会，牵手看电影")).toBe(false);
    expect(looksExplicit("向她表白然后求婚")).toBe(false);
    expect(looksExplicit("早点上床睡觉养足精神")).toBe(false); // 上床睡觉不是那个意思
  });
});
