...
只输出 JSON 数组，不要任何解释。每个行动的格式：
{
  ...
  "nsfw": 是否包含明确的性内容（布尔）。只有直接的性行为、露骨的亲密描写诉求才为 true；恋爱、约会、牵手、拥抱、接吻、表白、求婚、结婚、普通亲昵都为 false,
  "target": "涉及的具体人物姓名（没有则省略此字段）",
  "skill": "这件事在积累的技能名（2~6字的手艺名词，如 吉他/木工/编程/厨艺；纯玩乐、社交、无技能可积累则省略此字段）",
  "moneyCost": 这件事需要实际掏的钱（正整数；按当时年代的真实市价估算：手机数千、汽车数万起、房产数十万起；不花钱的事省略此字段）
}

规则：
- 最多拆 4 个行动；玩家写得再多也合并归纳。
- 先看角色状态里的金钱、年龄和身份，再解析：
  · 买东西必须按真实市价估 moneyCost，不要替玩家把价格压低——引擎会校验付不付得起，付不起会被判「无力承担」，这正是应有的现实感。
  · 未成年人的大额消费、远行通常需要家长点头，解析照常但 risk 标 high。
- 纯粹的愿望或宣言（"想去美国""想变有钱"）不是行动：把它落成角色当下真正可执行的准备动作（查资料、攒钱、学外语、打听门路），summary 写实际做的事；完全超出角色能力的空想标 risk:"high"。
- 明显不符合角色当前年龄/处境的要求也照常解析（引擎会判定失败），但把 risk 标为 high。
- 只输出 JSON 数组。`;
...
/**
 * 露骨信号词：判断一段文字是否带有明确的性内容线索。
 * 用于双信号路由——LLM 的 nsfw 标记必须与文本信号同时命中才把回合染色到 nsfw 后端，
 * 防止解析模型把普通恋爱内容乱标 nsfw、导致正常叙事整回合被送去 Grok。
 * 词表宁可略宽（信号命中只是"允许染色"，最终还要 LLM 标记同意）。
 */
const EXPLICIT_SIGNAL =
  /做爱|性爱|性交|性欲|情欲|欲火|春宵|云雨|翻云覆雨|滚床单|上床(?!睡)|开房|侍寝|交欢|欢爱|缠绵|肉体|赤身|赤裸|裸体|全裸|脱光|宽衣解带|解开.{0,4}[衣扣裙]|内衣|内裤|酥胸|乳房|乳头|私处|下体|下身|腿间|高潮|呻吟|娇喘|挑逗|爱抚|舌吻|湿吻|口交|自慰|春梦|春心|骚|淫|妓|嫖|援交|一夜情|炮友|约炮|床笫|鱼水之欢|共度良宵|肌肤之亲|sex|fuck|nsfw/i;

export function looksExplicit(text: string): boolean {
  return EXPLICIT_SIGNAL.test(text);
}
