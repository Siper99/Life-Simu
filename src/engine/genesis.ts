...
import { applyTalentToBounds, newbornAttributes, rollAttributeBounds } from "./attributes";
...
function makeParent(
  ...
  return {
    ...
    relation,
    birthYear: childBirthYear - rng.int(relation === "父亲" ? 24 : 22, relation === "父亲" ? 42 : 39),
    occupation: rng.pick(jobs),
    health: rng.int(55, 88),
    conditions: [],
...
export function rollGenesis(rng: Rng, genderPref: GenderPref = "random"): GenesisRoll {
  ...
  const attrBounds = rollAttributeBounds(rng);
  const attrs = newbornAttributes(rng, attrBounds);

  const father = makeParent(rng, "父亲", surname, fam.name, birthYear, pickGiven);
  const mother = makeParent(rng, "母亲", surname, fam.name, birthYear, pickGiven);
  ...
  for (let i = 0; i < siblingCount; i++) {
    ...
    const siblingBirthYear = birthYear - rng.int(1, 8);
    const siblingAge = birthYear - siblingBirthYear;
    npcs.push({
      id: `npc-sibling-${i}`,
      name: `${surname}${pickGiven(isBrother ? MALE_NAMES : FEMALE_NAMES)}`,
      relation: isBrother ? "哥哥" : "姐姐",
      birthYear: siblingBirthYear,
      occupation: siblingAge >= 6 ? "学生" : null,
      health: rng.int(65, 92),
      conditions: [],
  ...
  const background: Background = {
    ...
    familyWealth,
    familyDesc: `${fam.desc}。父亲是${father.occupation}（${father.personality.join("、")}），母亲是${mother.occupation}（${mother.personality.join("、")}）${siblingCount > 0 ? `，有${siblingCount}个兄弟姐妹` : "，是独生子女"}。`,
  };
...
export function applyTalent(character: CharacterState, talent: Talent): void {
  ...
  for (const [k, v] of Object.entries(talent.attrMods)) {
    ...
    applyTalentToBounds(character, key, v ?? 0);
  }
}
