...
- `scene.ts`：场景模式——镜头从「一季」拉近到「此刻」的连续对手戏：时间冻结、每拍扣 5 精力（上限 12 拍），玩家一拍台词/动作、`narrateSceneBeat` 接一拍剧情（NSFW 场景全程走 nsfw 后端并带完整场景记录保证连续性）；`settleScene` 收场结算好感/心境/共同记忆。收场时露骨场景的记忆同样只留替身文案。
