...
- `decisions.ts`：决策盘——`STAGE_POOL` 固定卡池（每张卡带 `variants` 变体文案跨回合轮换、`when` 条件闸门：无业才有求职卡等）、`ERAS` 时代底色 + `EPOCH_EVENTS` 大事年表（按国家优先匹配，命中年份 `WorldPulse.major=true` 并接管行动修正）、导演系统（读最近 4 回合选择倾向插卡，台词也轮换）、`contextualChoices` 情境卡（欠债/亲人病重/关系裂痕/技能磨砺/闲钱/线头回响，引用真实姓名与数字，用独立 RNG 保证 LLM 卡到达后日常卡抽样不变）、`personalizeDraft`（"才艺/专业技能"占位词按角色种子换成稳定的个人特长）、关系卡在好感前三里轮换对象且文案按关系类型/冷热定制、恢复卡按健康/精力处境换说法、机会卡每窗口换模板、主体卡区每回合洗牌、`boardHeadline`（情境化标题 + 默认问句轮换池）、`sanitizeLlmChoices`。所有轮换都从 `seed^turn` 派生：同回合稳定、跨回合变化。
...
- `orchestrator.ts`：`parseIntents` / `narrateTurn` / `narrateSceneBeat` / `proposeChoices` / `narrateSkip` / `writeEpitaph`，全部带离线兜底。
- **金钱现实感**：意图解析按真实市价估 `moneyCost`（提示词要求先看钱包/年龄/身份再解析，愿望型输入落成可执行的准备动作；离线走 `fallbackParseIntents` 的关键词价目表）；引擎在 `finalizeTurn` 做硬闸门——标价超过钱包的行动被 `unaffordableResolution` 拦成「无力承担」（不扣钱、不转针、只有落空心境），付得起的按真实标价扣款。

API key 在 Tauri 侧存系统凭据管理器（`secret_get`/`secret_set` invoke），设置 JSON 里不落盘；Rust 侧 `validate_profile_id` 对标识符做白名单校验。密钥发送前经 `client.ts # checkBackendUrl` 校验：明文 HTTP 只放行本机地址，远端必须 HTTPS；设置页保存时对非官方自定义域名弹「密钥将发往此地址」确认。发布流水线（`release.yml`）所有 Action 固定到完整 commit SHA、经受保护 `release` Environment 人工审批（保护规则需在 GitHub 网页端配置）。
